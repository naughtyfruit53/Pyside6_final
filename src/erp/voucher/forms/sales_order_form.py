# src/erp/voucher/forms/sales_order_form.py

from src.erp.logic.utils.forms_utils import *
from src.erp.logic.utils.sequence_utils import get_next_sales_order_sequence, increment_sales_order_sequence

class SalesOrderForm(QWidget):
    def __init__(self, parent=None, app=None, module_name=None, voucher_type_id=None, voucher_type_name=None, voucher_data=None, voucher_management=None, voucher_category=None, voucher_name=None, save_callback=None, add_product_callback=None, entities=None, products=None, payment_terms=None):
        ...
        self.voucher_type_name = voucher_type_name if voucher_type_name else "Sales Order"
        ...
        self.setObjectName("SalesOrderForm")
        apply_stylesheet(self, "sales_order_form.qss")
        self.setup_ui()

    def setup_ui(self):
        logger.info(f"Creating Sales Order form (ID: {self.voucher_type_id})")
        ...
        # Title
        title_label = create_title_label("Create Sales Order")
        self.content_layout.addWidget(title_label)

        # Header row: Voucher Number, Date, Required by Date
        header_fields = [
            ("Voucher Number*", 'text', "Voucher Number", get_next_sales_order_sequence() or ''),
    ...
    def save_voucher(self):
        save_voucher(self, ["Voucher Number", "Voucher Date", "Party Name"], increment_sales_order_sequence, stock_check=True, stock_update=True)
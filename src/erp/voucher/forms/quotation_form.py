# src/erp/voucher/forms/quotation_form.py

from src.erp.logic.utils.forms_utils import *
from src.erp.logic.utils.sequence_utils import get_next_quote_sequence, increment_quote_sequence
...
class QuotationForm(QWidget):
    def __init__(self, parent=None, app=None, module_name=None, voucher_type_id=None, voucher_type_name=None, voucher_data=None, voucher_management=None, voucher_category=None, voucher_name=None, save_callback=None, add_product_callback=None, entities=None, products=None, payment_terms=None):
        ...
        self.voucher_type_name = voucher_type_name if voucher_type_name else "Quotation"
        ...
        self.setObjectName("QuotationForm")
        apply_stylesheet(self, "quotation_form.qss")
        self.setup_ui()

    def setup_ui(self):
        logger.info(f"Creating Quotation form (ID: {self.voucher_type_id})")
        ...
        # Title
        title_label = create_title_label("Create Quotation")
        self.content_layout.addWidget(title_label)

        # Header row: Voucher Number, Date, Valid Until
        header_fields = [
            ("Voucher Number*", 'text', "Voucher Number", get_next_quote_sequence() or ''),
            ("Date*", 'date', "Voucher Date", self.voucher_data.get("Voucher Date", QDate.currentDate().toString("yyyy-MM-dd"))),
            ("Valid Until", 'date', "Valid Until", self.voucher_data.get("Valid Until", QDate.currentDate().toString("yyyy-MM-dd")))
        ]
    ...
    def save_voucher(self):
        save_voucher(self, ["Voucher Number", "Voucher Date", "Party Name"], increment_quote_sequence)
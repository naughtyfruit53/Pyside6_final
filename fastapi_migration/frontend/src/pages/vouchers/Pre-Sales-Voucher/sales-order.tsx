...
const SalesOrderPage: React.FC = () => {
  ...
  const { control, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm({
    defaultValues: {
      voucher_number: 'SO/2526/00000001',
  ...
  const { data: voucherList, isLoading: isLoadingList } = useQuery(
    ['salesOrders'],
    () => voucherService.getSalesOrders() // Assume this method exists
  );
  ...
  const { data: voucherData, isLoading: isFetching } = useQuery(
    ...
    () => voucherService.getSalesOrderById(selectedId!),
    { enabled: !!selectedId }
  );
  ...
  const createMutation = useMutation((data: any) => voucherService.createSalesOrder(data), {
    onSuccess: () => {
  ...
  const updateMutation = useMutation((data: any) => voucherService.updateSalesOrder(selectedId!, data), {
    onSuccess: () => {
      queryClient.invalidateQueries('salesOrders');
      setMode('view');
    },
        ...
        <Grid item xs={12} md={7} lg={7.2}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="h4" gutterBottom>
              {mode === 'create' ? 'Create' : mode === 'edit' ? 'Edit' : 'View'} Sales Order
            </Typography>

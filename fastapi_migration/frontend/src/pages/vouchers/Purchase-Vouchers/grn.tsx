...
const GrnPage: React.FC = () => {
  ...
  const { control, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm({
    defaultValues: {
      voucher_number: 'GRN/2526/00000001',
  ...
  const { data: voucherList, isLoading: isLoadingList } = useQuery(
    ['grns'],
    () => voucherService.getGrns() // Assume this method exists
  );
  ...
  const { data: voucherData, isLoading: isFetching } = useQuery(
    ['grn', selectedId],
    () => voucherService.getGrnById(selectedId!),
    { enabled: !!selectedId }
  );
  ...
  const createMutation = useMutation((data: any) => voucherService.createGrn(data), {
    onSuccess: () => {
  ...
  const updateMutation = useMutation((data: any) => voucherService.updateGrn(selectedId!, data), {
    onSuccess: () => {
      queryClient.invalidateQueries('grns');
      setMode('view');
    },
        ...
        <Grid item xs={12} md={7} lg={7.2}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="h4" gutterBottom>
              {mode === 'create' ? 'Create' : mode === 'edit' ? 'Edit' : 'View'} GRN
            </Typography>

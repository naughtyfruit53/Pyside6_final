...
const SalesVoucherPage: React.FC = () => {
  ...
  const { control, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm({
    defaultValues: {
      voucher_number: 'SV/2526/00000001',
  ...
  const { data: voucherList, isLoading: isLoadingList } = useQuery(
    ['salesVouchers'],
    () => voucherService.getSalesVouchers() // Assume this method exists
  );
  ...
  const { data: voucherData, isLoading: isFetching } = useQuery(
    ...
    () => voucherService.getSalesVoucherById(selectedId!),
    { enabled: !!selectedId }
  );
  ...
  const createMutation = useMutation((data: any) => voucherService.createSalesVoucher(data), {
    onSuccess: () => {
  ...
  const updateMutation = useMutation((data: any) => voucherService.updateSalesVoucher(selectedId!, data), {
    onSuccess: () => {
      queryClient.invalidateQueries('salesVouchers');
      setMode('view');
    },
